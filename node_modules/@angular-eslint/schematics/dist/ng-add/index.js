...
const angular_version_compat_1 = require("../angular-version-compat");
...
function applyESLintConfigIfSingleProjectWithNoExistingTSLint(options) {
    return (host, context) => {
        const tseslintPreset = (0, utils_1.resolveTseslintPreset)(options.tseslintPreset);
        const setParserOptionsProject = (0, utils_1.shouldEnableProjectService)(options.setParserOptionsProject ?? false, tseslintPreset);
        ...
        if (projectNames.length === 0) {
            (0, utils_1.warnIfTypeCheckedPreset)(context, tseslintPreset);
            return (0, schematics_1.chain)([
                (tree) => {
                    // If the root package.json uses type: module, generate ESM content
                    const packageJson = (0, utils_1.readJsonInTree)(tree, 'package.json');
                    const isESM = packageJson.type === 'module';
                    tree.create('eslint.config.js', (0, utils_1.createStringifiedRootESLintConfig)(null, isESM, {
                        tseslintPreset,
 ...
 */
function assertAngularVersionCompatibility(workspacePackageJson, context) {
    const expectedMajor = (0, angular_version_compat_1.parseMajorVersion)(packageJSON.version);
    if (expectedMajor == null) {
        return;
    }
    const mismatches = (0, angular_version_compat_1.findAngularVersionMismatches)(workspacePackageJson, expectedMajor);
    if (mismatches.length > 0) {
        throw new schematics_1.SchematicsException((0, angular_version_compat_1.formatAngularVersionMismatchError)(expectedMajor, mismatches));
    }
    if ((0, angular_version_compat_1.hasDetectableAngularVersion)(workspacePackageJson)) {
        context.logger.info((0, angular_version_compat_1.formatAngularVersionMatchMessage)(expectedMajor));
    }
}
/**
